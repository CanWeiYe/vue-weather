<template>
  <div id="search-container">
        ...
        <div class="search">
          ...
          <div class="s-bar">
            <form>
              <input type="text" ref="keyword" value="搜索城市，如：北京" onfocus="this.value = '';" onblur="if (this.value.trim() == '') {this.value = '搜索城市，如：北京';}"
               >
              <input type="submit"  value="搜索" @click="searchWeather"/>
            </form>
          </div>
          <p>您也可以: <a href="#" style="font-size: 1.1em" @click="searchWeatherByIp">获取当前城市天气</a></p>
        </div>
        <!--search end here-->
        <div class="copyright">
          <p>2020 &copy Spring Weather rights reserved</p>
        </div>
  </div>
</template>
...
<style scoped>
  ...
  /*end reset*/
  #search-container {
    background: url(../../assets/jpg/bg.jpg) no-repeat;
  ...
  .search i {
    background: url(../../assets/png/search.png) no-repeat;
    width: 31px;
    height: 31px;
    display: block;
    position: absolute;
